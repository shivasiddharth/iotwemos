...
//Change "/feeds/AlexaAndHome" to "/feeds/YOUR_FEED_NAME"
Adafruit_MQTT_Subscribe FEEDNAME = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/AlexaAndHome");
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(5000))) {
    ...
    if (subscription == &FEEDNAME) {
      Serial.print(F("Got from your Feed: "));

      Serial.println((char *)FEEDNAME.lastread);

      if (strcmp((char *)FEEDNAME.lastread, "on monitor") == 0) {
        mySwitch.send(13911311, 24);
        }
      if (strcmp((char *)FEEDNAME.lastread, "off monitor") == 0) {
